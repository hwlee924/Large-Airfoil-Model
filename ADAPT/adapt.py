...
class gp_layer(gpytorch.models.ApproximateGP):
    def __init__(self, inducing_points, nn_dims): 
        ...
        # Covariance module 
        self.covar_module = gpytorch.kernels.ScaleKernel(
            gpytorch.kernels.keops.MaternKernel(nu=3/2, ard_num_dims=nn_dims[-1], 
...
class DKL_model(gpytorch.Module): 
        def __init__(self, inducing_points, likelihood, data_dim, nn_dims, output_device='cpu', model_version='pca'): # train_x, train_y, likelihood
            super(DKL_model, self).__init__()
            
            # NN Feature Extractor Module
            self.feature_extractor = nn_layer(data_dim=data_dim, nn_dims=nn_dims, model_version=model_version)
        ...
        def load_weights(self, weight_num):
            ...
            if self.model_version == 'v2':
                # 
                print('Deprecated, come back to this')
            elif self.model_version == 'pca':
                pca_weights_path = Path(__file__).resolve().parent / 'model' / 'svdkl_pcaInput_weights.pt'
                checkpt = torch.load(pca_weights_path, map_location='cpu', weights_only=False);   
            self.load_state_dict(checkpt['model_state_dict'], strict=False)
...
class nn_layer(torch.nn.Sequential): # use this for LWM
    def __init__(self, data_dim, nn_dims, model_version):
        ...
        if model_version=='v2':
            ...
            # # Layer 4 
            self.add_module('linear4', torch.nn.Linear(nn_dims[2], nn_dims[3]))
            self.add_module('relu4', torch.nn.LeakyReLU())
            self.add_module('dropout4', torch.nn.Dropout(0.3))
            
            # Layer 5 
            self.add_module('linearx', torch.nn.Linear(nn_dims[3], nn_dims[4]))
            self.add_module('relux', torch.nn.LeakyReLU())
            self.add_module('dropoutx', torch.nn.Dropout(0.3))
            
            # Layer 6
            self.add_module('linearxx', torch.nn.Linear(nn_dims[4], nn_dims[5]))
            self.add_module('reluxx', torch.nn.LeakyReLU())
            self.add_module('dropoutxx', torch.nn.Dropout(0.3))
            
            # Output layer 
            self.add_module('linear5', torch.nn.Linear(nn_dims[5], nn_dims[6]))
...
#         # Output layer 
#         self.add_module('linear5', torch.nn.Linear(nn_dims[5], nn_dims[6])) # torch.nn.Linear(nn_dims[6], nn_dims[7])
...
class input_data():
    def __init__(self, airfoil_input, alpha, mach, num_auto_points:int=120, manual_points:torch.Tensor=None, output_device:torch.device=None, model_version:str='v1'): 
        ...
        if model_version == 'pca':
            ...
            with open(Path(__file__).resolve().parent / 'model' / 'airfoil_pca.pkl', 'rb') as file:
                ...
                self.pca_components = pca_data['pca_component']
                self.normalizer = torch.tensor([4.4, 1.4, 1.0, 0.54, 0.46, 0.3, 0.3, 0.3, 0.14, 0.08, 0.08, 0.08, 0.08, 0.08, 0.08])
    ...
    # set up PCA
    def pca_transform(self, input_tensor):
        transform_x = torch.matmul(input_tensor - self.pca_mean, self.pca_components.T)
        return transform_x

    def pca_inv_transform(self, input_pca_tensor):
        i_transform_x = torch.matmul(input_pca_tensor, self.pca_components) + self.pca_mean
    ...
    def assemble_tensor(self):
        ...
        if self.model_version=='v1':
            ...
            raw_geometry = torch.hstack((cols_af_u*10, cols_af_l*10)) - self.x_mean
            converted_to_pca = self.pca_transform(raw_geometry)
            out_tensor = torch.hstack((converted_to_pca, cols_alph, cols_mach, xhat[:,None], yhat[:,None]))
        else: 
...
def unpack_model(model_version:str='v2', output_device:torch.device=None, verbose:bool=False):
    ...
    with warnings.catch_warnings(): 
        ...
        if model_version=='v1': 
            ...
            model = DKL_model(inducing_points, likelihood, data_dim, nn_dims, model_version='v2')
            ...
            # Load in model & likelihood 
            model = DKL_model(inducing_points, likelihood, data_dim, nn_dims, model_version='pca')
            likelihood = likelihood
