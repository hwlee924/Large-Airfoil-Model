...
Original file is located at
    https://colab.research.google.com/drive/1Ajdufx_DarGD67P3dDzGNPllgJzTe5bP
"""

!pip install transformers pandas gpytorch==1.12 requests huggingface_hub sentence_transformers gradio_client
!pip3 install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cu125
...
class aspire_retriever:
    ...
    def extract_query_parameters(self, query, qa_pipeline):
        ...
        parameters = {
            ...
            "Angle of attack": "What is the angle of attack in degrees, allowing for optional negative degrees? It may be written as aoa. If the user doesn't explicitly mention the angle of attack separate from the other values, the value is None",
            "Mach": "What is the Mach number? It may be written as mach. If the user doesn't explicitly mention the Mach number separate from the other values, the value is None.",
            "Reynolds": "What is the Reynolds number or re? It may be written as reynolds or re. If the user doesn't explicitly mention the reynold's number separate from the other values, the value is None"
        }
        ...
        for param, question in parameters.items():
            try:
                ...
                if param == "Airfoil":
                    ...
                    if any(char.isdigit() for char in extracted_value) and (extracted_parameters["Airfoil"] is None or extracted_parameters["Airfoil"] not in extracted_value):
    ...
    def retrieve_context(self, query, last_airfoil=None, last_angle=None, last_mach=None):
        # retrieve the extracted query parameters
        query_params = self.extract_query_parameters(query, self.lambert)
        airfoil_storage = query_params["Airfoil"]
        num_none = 0

        if query_params["Airfoil"] is None:
          if last_airfoil is not None:
            ...
            query_params["Airfoil"] = last_airfoil_name
          else:
            return query_params["Angle of attack"], query_params["Mach"], None, 'There is no matching airfoil available in the database. You cannot retrieve the airfoil coordinates from the database. Maybe the attachment has information you can use.' # return empty string for no context
        if query_params["Angle of attack"] is None:
            if last_angle is not None:
                query_params["Angle of attack"] = last_angle
            else:
                num_none += 1
        if query_params["Mach"] is None:
            if last_mach is not None:
                query_params["Mach"] = last_mach
            else:
                num_none += 1
        ...
        if not query_params["Airfoil"]:
            return query_params["Angle of attack"], query_params["Mach"], None, ''
        ...
        if af_match_idx is not None:
            ...
            return airfoil_storage, query_params["Mach"], None, 'There is no matching airfoil available in the database. You cannot retrieve the airfoil coordinates from the database. Maybe the attachment has information you can use.' # return empty string for no context
        query_params["Airfoil"] = retrieved_airfoil
        print(f"[DEBUG] retrieved airfoil: {retrieved_airfoil}" if self.show_debug_logs else '', end='' if not self.show_debug_logs else '\n')
        for param in query_params:
            print(f"param {param} {query_params[param]}")
        ...
        if num_none <= 1:
            for metadata in metadata_list: # DO REYNOLDS AND FIX
                ...
                if angle_score < aspire_retriever.angle_threshold and mach_score < aspire_retriever.mach_threshold : # and re_score < re_threshold
        ...
        if best_match_idx is not None:
            return query_params["Angle of attack"], query_params["Mach"], retrieved_airfoil, retrieved_airfoil + self.data_contexts[af_match_idx][best_match_idx]
        else:
            return query_params["Angle of attack"], query_params["Mach"], retrieved_airfoil, retrieved_airfoil + ' No relevant pressure distribution file was found in the database. You should instead predict the results using the ADAPT module.'
...
class rag_model:
    ...
    def ask_query(self, web_input, attachment=None):
        ...
        airfoil_angle, airfoil_mach, airfoil_name, retrieved_context = self.retriever.retrieve_context(query, self.last_airfoil, self.last_angle, self.last_mach) # get context for RAG
        ...
        correct_response = input("Is this what you were looking for? (y/n)")
        if("y" in correct_response.lower()):
          self.memory.add_entry(query, response, retrieved_context)
          if airfoil_name is not None:
            self.last_airfoil = airfoil_name
          if airfoil_angle is not None:
            self.last_angle = airfoil_angle
          if airfoil_mach is not None:
            self.last_mach = airfoil_mach
        elif("n" in correct_response.lower()):
    ...
    def init_function_definitions(self):
        ...
        {
          "name": "change_thresholds",
          "description": "Changes the thresholds for angle of attack, mach number, and reynolds number. Invoke this function if the user asks to change thresholds.",
          "parameters": {
              "type": "object",
              "properties": {
    ...
    def init_model_instructions(self):
        self.instructions = f"""You are an AI-driven airfoil aerodynamics analysis digital assistant, named Large Airfoil Model Digital Assistant (LAMDA).
                ...
                Task 8: This is the task if the user asks to change thresholds for any of the values. In this task, change the thresholds. This will require invoking the function "change_thresholds".
...
# by explaining what it is and what file you obtained the information but .
    ...
    def retrieve_and_plot_geometry_from_csv(self, coordinates_file_url):
        ...
        if coordinates_file_url and os.path.isfile(coordinates_file_url): # utilizing attachment file if it exists
    ...
    def change_thresholds():
        new_angle_threshold = input("The current threshold for angle of attack is " + aspire_retriever.angle_threshold + " enter the new value: ")
        new_mach_threshold = input("The current threshold for mach number is " + aspire_retriever.mach_threshold + " enter the new value: ")
        #new_re_threshold = input("The current threshold for reynolds number is " + aspire_retriever.re_threshold + " enter the new value: ")
        aspire_retriever.angle_threshold = new_angle_threshold
        aspire_retriever.mach_threshold = new_mach_threshold
        #aspire_retriever.re_threshold = new_re_threshold
        print("Thresholds have been changed.")
...
class adapt_web_predictor():
    def __init__(self, conversation, show_debug_logs=False, use_gpu=True):
        self.use_gpu = use_gpu # gpu usage, inherit from main class
        self.show_debug_logs = show_debug_logs
        lam_token = userdata.get("lamToken")
...
if run_local:
    llama_token = userdata.get("HuggingFace")
    lam_da = rag_model(show_debug_logs=True, context_save_file='temp_model.pkl', online=True) # None
...
while True:
    ...
    dummy_json.add_query(input("Enter query: "))
